if (place_meeting(x , y, oKillbox)) { //Die if touching killbox
...
// Applying gravity
if (!place_meeting(x, y + 2, oGround)) { // If not touching ground
...
// Jumping 
if ((keyboard_check_pressed(vk_up) || keyboard_check_pressed(vk_space)) && place_meeting(x, y + 2, oGround)) {
...
if (vspeed < 0 && abs(hspeed) >= 0) { // Only apply jump sprite when stationary & airborne
    ...
    sprite_index = sPlayer_run_dark;
} 
else { // Standing still
    sprite_index = sPlayer_dark; 
}
